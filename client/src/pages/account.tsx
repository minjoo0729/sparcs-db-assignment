...
const AccountPage = () => {
  ...
  const getAccountInformation = () => {
    const asyncFun = async() => {
      interface IAPIResponse { balance: number, accountID: string };
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/getInfo', { credential: { id: SAPIID, pw: SAPIPW } });
  ...
  const performTransaction = ( amount: number | '' ) => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/transaction', { credential: { id: SAPIID, pw: SAPIPW}, amount: amount });
      setNTransaction(0);
      if (!data.success) {
  ...
  return (
    <div className={"account"}>
      ...
      <div className={"account-token-input"}>
        ID: <input type={"text"} value={SAPIID} onChange={e => setSAPIID(e.target.value)}/>
        PW: <input type={"password"} value={SAPIPW} onChange={e => setSAPIPW(e.target.value)}/>
